...
use diesel::{r2d2::{Pool, ConnectionManager, PooledConnection}, pg::PgConnection};
...
lazy_static! {
    pub static ref DBCONNECTION: DbConnection = {
        ...
        DbConnection {
            db_connection: PgPool::builder().max_size(8).build(ConnectionManager::new(connection_string)).expect("failed to create a database connection_pool")
        }
    };
}
